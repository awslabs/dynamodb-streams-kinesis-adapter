...
public class DynamoDBStreamsShutdownTask implements ConsumerTask {
    ...
    public DynamoDBStreamsShutdownTask(@NonNull ShardInfo shardInfo,
                                       @NonNull ShardDetector dynamoDBStreamsShardDetector,
                                       ...
                                       @NonNull InitialPositionInStreamExtended initialPositionInStream,
                                       boolean cleanupLeasesOfCompletedShards,
                                       boolean ignoreUnexpectedChildShards,
                                       @NonNull LeaseCoordinator leaseCoordinator,
                                       long backoffTimeMillis,
                                       @NonNull RecordsPublisher recordsPublisher,
                                       @NonNull HierarchicalShardSyncer dynamoDBStreamsShardSyncer,
        ...
        this.streamArn = KinesisMapperUtil
                .createDynamoDBStreamsArnFromKinesisStreamName(this.streamIdentifier.streamName());
    }
        ...
        try {
            ...
            if (reason == ShutdownReason.SHARD_END) {
                try {
                    ...
                    log.warn(
                            "Lease {}: Invalid state encountered while shutting down shardConsumer with SHARD_END "
                                    + "reason. " + "Dropping the lease and shutting down shardConsumer using "
                                    + "LEASE_LOST reason.",
                            leaseKey,
                            e);
    ...
    private void createLeasesForChildShardsIfNotExist(MetricsScope scope)
        ...
        String streamArn = KinesisMapperUtil.createDynamoDBStreamsArnFromKinesisStreamName(
        ...
        for (ChildShard childShard: childShards) {
            ...
            if (leaseRefresher.getLease(leaseKey) == null) {
                log.debug(
                        ...
                        dynamoDBStreamsShardDetector.streamIdentifier(),
                        shardInfo.shardId(),
                        leaseKey);
                final Lease leaseToCreate = dynamoDBStreamsShardSyncer
                        .createLeaseForChildShard(childShard, streamIdentifier);
                final long startTime = System.currentTimeMillis();
    ...
    private void updateLeaseWithChildShards(Lease currentLease)
        ...
        log.info(
                "Shard {} of Stream {}: Updated current lease {} with child shard information: {}",
                shardInfo.shardId(),
                streamArn,
    ...
    private void dropLease(Lease currentLease, final String leaseKey) {
        if (currentLease == null) {
            ...
            log.info("Dropped lease for shutting down ShardConsumer: {} of stream: {}",
                    currentLease.leaseKey(), streamArn);
        }
    }
}
